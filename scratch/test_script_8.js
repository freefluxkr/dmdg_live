    ...
    function openAudioPopup(mode, text = "", analyser = null) {
      ...
      if (mode === 'recording') {
        ...
        subtitle.textContent = "편안하게 마음속 이야기를 들려주세요.";
    ...
    function startPopupBlob(analyser) {
      ...
      function draw() {
        ...
        for (let i = 0; i < points; i++) {
          const angle = (i / points) * Math.PI * 2;
          const offset = Math.sin(angle * 3 + popupBlobTime) * 6 + Math.cos(angle * 2 - popupBlobTime * 0.5) * 4 + (avg > 0 ? (dataArray[i % bufferLength] / 255) * 15 : 0);
          const r = baseRadius + offset;
          const x = centerX + Math.cos(angle) * r;
          const y = centerY + Math.sin(angle) * r;
        ...
        try {
          ...
          mediaRecorder.onstop = async () => {
            try {
              ...
              const finalRecognizedText = recognitionText.trim() || originalText;

              tempOriginalText = originalText;
              tempRecognizedText = finalRecognizedText;
    ...
    async function openEvaluationModal(originalText, recognizedText) {
      ...
      try {
        ...
        // Gemini의 피드백 문자열(개선할 점)을 시각화 영역에 크게 노출
        const feedbackVisual = document.getElementById('evaluation-visual-feedback');
        feedbackVisual.innerHTML = `<div class="p-3 bg-rose-50 rounded-xl border border-rose-100 shadow-sm text-center">
    ...
    async function translateTextWithGemini(text, targetLang) {
      ...
      if (!apiKey) {
        showToast(i18n[currentLang].gemini_key_needed || "Gemini API Key를 먼저 설정해 주세요.");
        openSettingsModal();
    ...
    async function triggerPrimerTranslation() {
      ...
      try {
        translationText.innerHTML = `<span class="animate-pulse">${i18n[currentLang].translate_loading}</span>`;
        translationBox.classList.remove('hidden');

        const translated = await translateTextWithGemini(primerText, targetTranslationLang);
        
        translationText.textContent = translated;
        
        // Cache translation locally
        if(activePrimer) {
            if(!activePrimer.translation) activePrimer.translation = {};
            activePrimer.translation[targetTranslationLang] = translated;
        }
      } catch (err) {
        console.error("번역 에러:", err);
        translationText.textContent = "번역 중 오류가 발생했습니다.";
      }</span>`;
        lucide.createIcons();
      }
    }
    ...
    async function processVoiceDonation() {
      ...
      try {
        ...
        loadRelays();
        openEvaluationModal(tempOriginalText, tempRecognizedText);
        updateWarmthLogo();
        startMilkyWayEffect();
      } catch (localErr) {
